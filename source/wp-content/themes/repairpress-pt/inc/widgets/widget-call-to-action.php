...
if ( ! class_exists( 'PW_Call_To_Action' ) ) {
	class PW_Call_To_Action extends WP_Widget {
		...
		function widget_name() { return esc_html__( 'Call to Action', 'repairpress-pt' ); }
		function widget_description() { return esc_html__( 'Call to Action widget for Page Builder.', 'repairpress-pt' ); }
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'subtitle' ) ); ?>"><?php _ex( 'Subtitle:', 'backend', 'repairpress-pt' ); ?></label>
				<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'subtitle' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'subtitle' ) ); ?>" type="text" value="<?php echo esc_attr( $subtitle ); ?>" />
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'button_text' ) ); ?>"><?php _ex( 'Button Area:', 'backend', 'repairpress-pt' ); ?></label>
				...
				<span class="button-shortcodes">
					<?php printf( _x( 'Input a button shortcode in the above field. Please take a look at the <a href="%1$s" target="_blank">Buttons Shortcode documentation</a> to learn more on how to write button shortcodes.', 'backend', 'repairpress-pt' ), 'https://www.proteusthemes.com/docs/repairpress-pt/#buttons' ); ?><br>
				</span>
			</p>
