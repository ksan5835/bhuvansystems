...
if ( RepairPressHelpers::is_woocommerce_active() ) {
	 ...
	 */
	add_action( 'woocommerce_before_main_content', 'repairpress_theme_wrapper_start', 10 );
	add_action( 'woocommerce_after_main_content', 'repairpress_theme_wrapper_end', 10 );

	function repairpress_theme_wrapper_start() {
		$sidebar = repairpress_get_shop_sidebar();
	...
	<div class="content-area  container">
		<div class="row">
			<main id="main" class="site-main  col-xs-12<?php echo 'left' === $sidebar ? '  col-md-9  col-md-push-3' : ''; echo 'right' === $sidebar ? '  col-md-9' : ''; ?>" role="main">
				...
				function repairpress_theme_wrapper_end() {
					$sidebar = repairpress_get_shop_sidebar();
				?>
			</main>
			<?php if ( 'none' !== $sidebar ) : ?>
	 ...
	 * @return array
	 */
	function repairpress_woo_body_class( $classes ) {
	...
	add_filter( 'body_class', 'repairpress_woo_body_class', 11 );

	// Display custom number of products per page
	function repairpress_custom_number_of_products_per_page( $cols ) {
		return absint( get_theme_mod( 'products_per_page', '9' ) );
	}
	add_filter( 'loop_shop_per_page', 'repairpress_custom_number_of_products_per_page', 20 );
	 ...
	 * @return array
	 */
	function repairpress_output_related_products_args( $args ) {
		$args['posts_per_page'] = 3;
		$args['columns']        = 3;

		return $args;
	}
	add_filter( 'woocommerce_output_related_products_args', 'repairpress_output_related_products_args' );

	/**
	 * Get the position of the sidebar for the shop pages, conditionally for the single product
	 */
	function repairpress_get_shop_sidebar() {
		if ( is_product() ) {
	 ...
	 */
	if ( ! function_exists( 'repairpress_loop_columns' ) ) {
		function repairpress_loop_columns() {
			return 3; // 3 products per row
		}
	}
	add_filter( 'loop_shop_columns', 'repairpress_loop_columns' );
